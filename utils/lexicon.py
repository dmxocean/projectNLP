...
def load_lexicon(lexicon_type, lang="ALL"):
    ...
    path = os.path.join(
        base_path, "data", "lexicons", lexicon_type
    )  # Full path to lexicon dir
...
def organize_by_POS(lexicon_df):
    ...
    for _, row in lexicon_df.iterrows():
        pos = (
            row["POS"] if "POS" in row else "other"
        )  # Default to "other" if POS missing
        term = row["term"]
        if pos in organized:
...
def is_uncertainty(word, lang="ALL"):
    ...
    if any(
        word in term.lower().split() for term in lexicon_df["term"]
    ...
    if nlp_es is None or nlp_ca is None:
        ...
        try:  # Try loading existing models
            ...
            try:  # Download models using subprocess
                ...
                subprocess.check_call(
                    [sys.executable, "-m", "spacy", "download", "ca_core_news_sm"]
                )
...
def detect_language(text, auto=False, interactive=False, default_lang="es"):
    ...
    try:  # Try by checking existing classifications
        from words import allclassifications
    ...
    if auto:  # Try spaCy-based detection
        try:
            ...
            es_recognized = sum(
                1 for token in doc_es if not token.is_oov
            )  # Count recognized tokens
            ca_recognized = sum(1 for token in doc_ca if not token.is_oov)

            if (
                ca_recognized > es_recognized
            ):  # If more Catalan tokens recognized, return "ca"
                return "ca"
            else:  # If equal or more Spanish tokens recognized, return "es"
                return "es"
        except Exception as e:
            print(
                f"Error in automatic language detection: {e}"
            )  # Fall through to next method instead of returning
...
def process_lexicons_with_language(
    neg_lexicon, unc_lexicon, force_rebuild=False, append_new=True
):
    """
    ...
    if words_exist:
        ...
        try:
            with open(
                words_path, "r"
            ) as f:  # Read the file directly to get all entries including duplicates
                content = f.read()

            # Extract pairs using regex for both dictionaries
            neg_entries = re.findall(
                r"negation\s*=\s*{.*?}", content, re.DOTALL
            )  # Allow newlines
            ...
            for _, row in neg_lexicon.iterrows():
                term = row["term"]
                term_lower = term.lower()

                if term_lower in neg_term_lower_to_langs:
                    langs = neg_term_lower_to_langs[
                        term_lower
                    ]  # Get all language classifications for this term
                    for lang in (
                        langs
                    ):  # Create separate entry for each language to preserve duplicates
                        row_data = row.to_dict()
                        row_data["language"] = lang
                        neg_lexicon_expanded.append(row_data)
                else:
                    lang = detect_language(
                        term, auto=True, interactive=False
                    )  # Auto-detect for new terms
                    row_data = row.to_dict()
                    row_data["language"] = lang
                    neg_lexicon_expanded.append(row_data)

            neg_lexicon = pd.DataFrame(
                neg_lexicon_expanded
            )  # DataFrame with expanded entries

            unc_lexicon_expanded = []  # Process uncertainty terms
            for _, row in unc_lexicon.iterrows():
                term = row["term"]
                term_lower = term.lower()
            ...
            unc_lexicon = pd.DataFrame(
                unc_lexicon_expanded
            )  # DataFrame with expanded entries

            # DEBUG - Check if we preserved duplicates
            term_counts = neg_lexicon["term"].value_counts()
            print(
                f"Terms appearing multiple times in negation lexicon: {sum(term_counts > 1)}"
            )
            term_counts = unc_lexicon["term"].value_counts()
            print(
                f"Terms appearing multiple times in uncertainty lexicon: {sum(term_counts > 1)}"
            )

            if append_new and (
                new_neg_words or new_unc_words
            ):  # New words detected and append enabled
                ...
                for i, term in enumerate(new_neg_words):
                    ...
                    if lang == "both":
                        new_neg_classifications.append(
                            (term, "es")
                        )  # Add Spanish entry
                        new_neg_classifications.append(
                            (term, "ca")
                ...
                for i, term in enumerate(new_unc_words):
                    ...
                    if lang == "both":
                        new_unc_classifications.append(
                            (term, "es")
                        )  # Add Spanish entry
                        new_unc_classifications.append(
                            (term, "ca")
                ...
                with open(words_path, "w") as f:
                    ...
                    for (
                        term,
                        lang,
                    ) in new_neg_classifications:  # Write new classifications
                        f.write(f'    "{term}": "{lang}",\n')
                    f.write("}\n\n")
                    ...
                    for (
                        term,
                        lang,
                    ) in new_unc_classifications:  # Write new classifications
                        f.write(f'    "{term}": "{lang}",\n')
                    f.write("}\n\n")
                ...
                terms_with_both_neg = sum(
                    1
                    for _, langs in neg_term_lower_to_langs.items()
                    if "es" in langs and "ca" in langs
                )
                terms_with_both_unc = sum(
                    1
                    for _, langs in unc_term_lower_to_langs.items()
                    if "es" in langs and "ca" in langs
                )
        ...
        print(
            "No existing classifications found. Starting manual classification."
        )  # First run setup
        ...
        print(
            f"Detecting languages for {len(unc_lexicon['term'])} uncertainty terms..."
        )
